...
const view = {
  ...
  getCardElement(index) {
    return `<div data-number="${index}" class="card back"></div>`
  },
...
const controller = {
      ...
      case GAME_STATE.SecondCardAwaits:
        ...
        if (model.isRevealedCardsMatched()) {
          ...
          if (model.score === 260) {
            ...
            view.showGameFinished()  //遊戲結束
            return
          }
          this.currentState = GAME_STATE.FirstCardAwaits  //回初始狀態
        } else {
          // 配對失敗
  ...
  resetCards() {
    view.flipCards(...model.revealedCards) //把牌翻回來
    model.revealedCards = []  //清空
    controller.currentState = GAME_STATE.FirstCardAwaits  //回初始狀態
  }
}
